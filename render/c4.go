...
func (c *C4Renderer) Render(g *reqflow.Graph, w io.Writer) error {
	...
	for pkg, nodeIDs := range g.Clusters {
		...
		for _, id := range nodeIDs {
			node := g.Nodes[id]
			if node.Kind == reqflow.KindInfra {
				infraNodes = append(infraNodes, id)
				continue // Draw infra outside boundary
			}
			
			tech := "Go Struct"
			if node.Kind == reqflow.KindInterface {
				tech = "Go Interface"
			}
			
			format := "Component"
			if node.Kind == reqflow.KindStore {
				format = "ComponentDb"
			}
	...
	for _, edge := range g.Edges {
		relTxt := "Uses"
		if edge.Kind == reqflow.EdgeImplements {
			relTxt = "Implements"
		} else if edge.Kind == reqflow.EdgeEmbeds {
			relTxt = "Embeds"
		}
